...
ModelFactory.prototype.findDelta = function findDelta() {
  ...
  const addToKeys = Object.keys(addTo);
  for (let i = 0; i < addToKeys.length; i += 1) {
    const prop = addToKeys[i];
    const item = addTo[prop];
    const citem = typeof cache[prop] !== 'undefined' ? cache[prop] : false;
  ...
  for (let i = 0; i < cacheKeys.length; i += 1) {
    const prop = cacheKeys[i];
    const item = typeof addTo[prop] === 'undefined';
    if (item) {
      changeCounter += 1;
